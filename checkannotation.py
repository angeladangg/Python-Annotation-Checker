    ...
    def check(self,param,annot,value,check_history=''):
        ...
        def check_else(par,ano,val):
            ...
            if type(ano) == str and type(par) == dict:
                ...
                try:
                    if not eval(ano):
                        raise AssertionError(f"\'{k}\' failed annotation check(str predicate: \'{ano}\') args for evaluation: {s}")
                except NameError:
                    raise AssertionError(f"\'{k}\' failed annotation check(str predicate: \'{ano}\') args for evaluation: {s}")
                ...
                try:
                    ...
                    if not eval(ano):
                        raise AssertionError(f"\'{par}\' failed annotation check(str predicate: \'{ano}\') args for evaluation: {par} -> {val}")
                except NameError:
                    raise AssertionError(f"\'{par}\' failed annotation check(str predicate: \'{ano}\') args for evaluation: {par} -> {val}")
